...
import numpy as np
for image_name in os.listdir("RefImgs_Batch1_10bit"):
    image = cv2.imread("RefImgs_Batch1_10bit/" + image_name, -1)
    scale_factor = 254/np.max(image)
    image = (image * scale_factor).astype("uint8")
    plt.imshow(image)
    plt.colorbar()
    plt.show()
    cv2.imwrite("UINT8_B1/" + image_name, image)
