...
def add_gauss_noise(sequence, mean, sd, int_reg_center=None, int_rad=None, flank_mask=None):
    ...
    for image in sequence:
        ...
        raw_noise.append(np.round(noise, 0).astype(np.uint8))
        #noisy_sequence.append(noise.astype(np.uint8))
        #show(np.round(noisy, 0).astype(np.uint8))
    return noisy_sequence, raw_noise
...
def calculate_optical_flow_pair_LK(i1, i2, n=1, plot=False, max_level=0, win_size=(21, 21), ev_filtering=False, min_eig_threshold=None):
    ...
    if ev_filtering == True:
        updated_points, status, error = cv2.calcOpticalFlowPyrLK(prevImg=i1, nextImg=i2, prevPts=points_to_track, nextPts=None, maxLevel=max_level, winSize=win_size, flags=cv2.OPTFLOW_LK_GET_MIN_EIGENVALS, minEigThreshold=min_eig_threshold)
    else:
        updated_points, status, error = cv2.calcOpticalFlowPyrLK(prevImg=i1, nextImg=i2, prevPts=points_to_track,
                                                             nextPts=None, maxLevel=max_level, winSize=win_size)
    ...
    #Filter unsing status output
    flow[:,:,0] = np.where(status==1, flow[:,:,0], 0)
    flow[:, :, 1] = np.where(status == 1, flow[:, :,1], 0)
    #print(flow.shape)
...
def movement_eval(image, plume_mask, flow, threshold=1):
    ...
    masked_mean = np.ma.mean(np.ma.masked_where(plume_mask>0, flow_mag))

    return correct_prop, np.mean(flow_mag), masked_mean

def movement_eval_considering_noise(noise_flow, flow, plume_mask):
    #What is the 95th percentile of the movement generated by noise in the plume area
    noise_mag = flow_magnitude(noise_flow)
    plume_noise = np.ma.masked_where(plume_mask==0, noise_mag).compressed()
    if np.sum(plume_mask) >= 1:
        perc = np.percentile(plume_noise, 95)
        # What proportion of the plume has motion identified in excess of this
        flow_mag = flow_magnitude(flow)
        inxs = np.where(flow_mag >= perc, 1, 0)
        # show(inxs)
        result = np.ma.sum(np.ma.masked_where(plume_mask == 0, inxs)) / np.sum(plume_mask)

    else:
        perc = np.percentile(noise_mag, 95)
        result = np.nan
    return result, perc
