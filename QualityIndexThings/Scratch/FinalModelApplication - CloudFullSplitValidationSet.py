...
#In this example we apply the trained quality index models to the
#"Full Split" validation set for precipitation, replicating the result
#presented in the associated write-up of this work.

#################################################
#####Key variables for this application###########
image_names_df = pd.read_csv("C:/Users/ggp24ash/Documents/Quality Index Write Up/Supplementary/ForORDA_AsCSV_Feb27th2026/PrecipitationFullSplit/Precipitation_Full_TestSeen.csv") #Spreadsheet containing name of each sample, plus associated timestep and off-band sample names
...
outputs_save_path = "FinalModelOutputs/"  #Predictions saved here
...
image_names_df.to_excel(outputs_save_path + "ImageNamesWithModelPredictions.xlsx")

#Quick check of the precipitation prediction accuracy
image_names_df = Functions.convert_outcome_to_binary(image_names_df, "precipitation")
true = image_names_df["precipitation_Yes"].to_numpy()
predicted = (image_names_df["precipiation_prediction"].to_numpy() >= 0.5)
correct = true == predicted
accuracy = np.mean(correct) * 100
print("Precipitation classification accuracy: " + str(np.round(accuracy, 2)))