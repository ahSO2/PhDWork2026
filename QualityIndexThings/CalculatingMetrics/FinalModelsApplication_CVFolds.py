...
if len(timesteps) == 3:
    model = Functions.get_triple_branched_resnet18(device)
else:
    model = Functions.get_single_branched_resnet18(device)
...
for chunk in range(1, n_chunks + 1):
    ...
    for index, obs in enumerate(iter(dataloader)):
            ...
            x_norm = Functions.scale_and_norm_batch(obs, len(timesteps), device)
            #Predict using the model, and move the output from the GPU if applicable
            predictions = model(x_norm).cpu().detach().numpy()
...
image_names_df[to_predict + "_prediction"] = all_predictions
