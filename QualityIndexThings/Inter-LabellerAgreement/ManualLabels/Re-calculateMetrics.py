...
#In this script, I re-calculate metrics of model perfromance, allowing the
#column indicating the "true" correct prediction value to vary. This allows
#the models to be evaluated against each labeller's decisions individually, and
#against the majority-vote label in each case.
...
##########################################################################
outputs_df = pd.DataFrame(columns=["labeller", "ACC", "ACC_L95", "ACC_U95",
...
for labeller in labellers:
    target_binary = results_df[predict + "_binary_" + labeller].to_numpy()
    target_level = results_df[predict + "_level_" + labeller].to_numpy()
